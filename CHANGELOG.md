...
- Added vendor-neutral OpenTelemetry (OTLP/HTTP) log forwarding. Log events at or above the configurable `OTel_Log_Level__c` threshold are forwarded to any OTLP logs endpoint (Azure Monitor native OTLP ingestion, AWS CloudWatch's native OTLP endpoint, an OpenTelemetry collector, or any vendor backend) by the new `Send_Log_Event_to_OTel` flow action. Each event becomes an OTLP `logRecord` (severity mapped to the OTel `SeverityNumber` scale) under a single `resourceLogs` payload, sent in callouts of up to 100 events. The full logs URL and any backend-specific headers are configured on the `RFLIB_OTEL_LOGS` Named Credential. See `otel/README.md` for setup recipes including the Lambda-free AWS CloudWatch OTLP option.
