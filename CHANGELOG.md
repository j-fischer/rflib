...
- Added the `rflib_PlatformInfoTransformer` utility that flattens an event's `Platform_Info__c` (both the Apex governor-limit map and the browser `performance` payload) into namespaced, typed telemetry attributes (e.g. `rflib.apex.cpu_time`, `rflib.browser.timing.dom_complete`, `user_agent.original`) so the values are individually queryable in Application Insights and OpenTelemetry backends.
