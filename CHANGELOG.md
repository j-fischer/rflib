...
- Added the `rflib_PlatformInfoParser` utility that flattens an event's `Platform_Info__c` (both the Apex governor-limit map and the browser `performance` payload) into namespaced, typed telemetry attributes (e.g. `rflib.apex.cpu_time`, `rflib.browser.timing.dom_complete`, `user_agent.original`) so the values are individually queryable in Application Insights and OpenTelemetry backends.
- Refactored `rflib_HttpCalloutLogAction` (AWS CloudWatch / generic HTTP forwarding) onto a new shared abstract base class (`rflib_LogEventHttpForwarderAction`) that owns the log-level matching, batching, HTTP callout, truncation, and failure handling for bulk log forwarders. The existing action keeps its invocable signature, `RFLIB_HTTP_CALLOUT_LOG` Named Credential, and behavior unchanged; the App Insights and OpenTelemetry actions are new subclasses.
...
- Refactored the Slack and Microsoft Teams notification actions to share a common abstract base class (`rflib_LogEventNotificationAction`) that owns the log-level matching, per-invocation cap, HTTP callout, truncation, and failure handling. The channel-specific classes now only supply their endpoint, configuration accessors, and message payload.
