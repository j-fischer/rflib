...
Package Alias: RFLIB@11.2.0-1
Install link: https://login.salesforce.com/packaging/installPackage.apexp?p0=04tKY0000005SpvYAE

- Added a `Fields Without FLS` menu to the Permissions Explorer's field permission views, which switches those views between `Hidden` (the default) and `Shown`. Salesforce only stores a `FieldPermissions` record when a profile or permission set grants Read or Edit on a field whose access Field Level Security can control, so fields it cannot control — required fields, master-detail fields, and standard system fields such as `Name`, `OwnerId`, and `CreatedById` — never appear in these views at all. That made a missing field ambiguous: it could mean the profile has no access, or that Field Level Security does not govern the field in the first place. Set to `Shown`, those fields are listed for every object already in the current results, with Read access reported as `true` — such a field is readable whenever the object is readable, the same condition a granted field permission depends on — and Edit access reflecting whether the field can be written at all, so audit fields like `CreatedById` read as not editable while `Name` reads as editable. The rows are styled to set them apart from stored grants. Field metadata is only retrieved for the objects already listed in the current results, the view returns to `Hidden` whenever the permission type changes or the page reloads, and on large orgs the Explorer asks for confirmation before adding more than 50,000 rows to the table. CSV exports of field permissions include these rows whenever they are shown, and gain a trailing `FLS CONTROLLED` column — `true` for a stored field permission, `false` for a field Field Level Security cannot control — so the two remain distinguishable outside the dashboard.
- Added the optional `Permissions_Explorer_Show_Fields_Without_FLS` Global Setting, which selects the mode the `Fields Without FLS` menu starts in. Set the value to `true` to have every field permission view load these fields automatically as soon as the permission records have finished loading. Without the setting, and for any value other than `true`, the views keep starting with them hidden. The menu still switches the current view at any time; the setting only decides where it starts.
- Searching the Permissions Explorer now raises a `Searching Permissions` spinner that covers the dashboard until the results are ready, on result sets of 30,000 records or more. A search narrows the entire loaded result set at once, which on an org that size takes long enough for the page to stop responding to clicks with nothing on screen to explain the pause. The spinner reports the wait and blocks further input until the table has been rebuilt. Smaller result sets are unaffected — the search finishes quickly enough that a spinner would only flicker, so none is shown.
- Paging through the Permissions Explorer no longer slows down as the result set grows. Each page change previously re-applied the search and copied every loaded permission record in order to render a single page of rows, so on an org with tens of thousands of field permissions every click cost seconds, and roughly twice that with fields without FLS shown. The search is now re-applied only when the records or the search terms change, and only the rows of the current page are built, so a page change costs the same whether the org holds hundreds of permission records or a hundred thousand. The rows displayed are unchanged.
- Fixed `Aggregate Permissions` in the Permissions Explorer failing with an error on orgs where fewer permission records reached the browser than the org actually holds — which happens when the query cap truncates the result set, or when a follow-up fetch fails part way through and leaves partial data behind. The record count in the page header is now derived from the records actually loaded, so it agrees with the table in both cases.

### RFLIB 11.1.0

Package ID: 04tKY0000005SoOYAU
Package Alias: RFLIB@11.1.0-1
Install link: https://login.salesforce.com/packaging/installPackage.apexp?p0=04tKY0000005SoOYAU
...
Package Alias: RFLIB-TF@4.0.0-2
Install link: https://login.salesforce.com/packaging/installPackage.apexp?p0=04tKY0000005SoJYAU
...
- **BREAKING:** `rflib_RetryableActionHandler.execute()` now receives a `List<rflib_RetryableAction>` instead of a `List<rflib_Retryable_Action__e>`. The new read-only `rflib_RetryableAction` data object works for both Platform Events and exposes the custom fields as properties named after the field API names without the `__c` suffix (`Action`, `Record_ID`, `Additional_Record_ID`, `Additional_Details`), the standard event fields (`ReplayId`, `EventUuid`), and the raw event record via the `event` property. To migrate a handler, change the parameter type of `execute()` and drop the `__c` suffix from field accesses, e.g. `evt.Record_ID__c` becomes `action.Record_ID`. The code change alone is not enough: installing the package recompiles all Apex in the org, so an existing handler blocks the install, while the new signature cannot be deployed before the package is installed. Follow the phased deployment in [Upgrading to RFLIB-TF 4.0.0](https://github.com/j-fischer/rflib/wiki/Getting-Started-with-Retryable-Actions#upgrading-to-rflib-tf-400).
- **BREAKING:** Raised the minimum package dependencies to `RFLIB@10.0.0-1` (from `RFLIB@4.1.0-1`) and `RFLIB-FS@4.0.0-1` (from `RFLIB-FS@2.0.0-1`). RFLIB-TF 4.0.0 now requires RFLIB 10.0.0 or later and RFLIB-FS 4.0.0 or later in the target org; install or upgrade those before upgrading RFLIB-TF.

### RFLIB-FS 4.0.0

Package ID: 04tKY0000005SfyYAE
Package Alias: RFLIB-FS@4.0.0-1
Install link: https://login.salesforce.com/packaging/installPackage.apexp?p0=04tKY0000005SfyYAE

- **BREAKING:** Raised the minimum RFLIB dependency to `RFLIB@10.0.0-1` (from `RFLIB@4.1.0-1`). RFLIB-FS 4.0.0 requires RFLIB 10.0.0 or later in the target org — install or upgrade RFLIB to at least 10.0.0 before upgrading RFLIB-FS. There are no functional changes to the Feature Switches package itself.
...
- Added Azure Application Insights log forwarding. Log events at or above the configurable `App_Insights_Log_Level__c` threshold are forwarded to Azure Application Insights via the telemetry ingestion endpoint (`v2.1/track`) by the new `Send_Log_Event_to_App_Insights` flow action. Each event becomes a `Microsoft.ApplicationInsights.Message` envelope (severity mapped to the App Insights scale) and the batch is sent as newline-delimited JSON in callouts of up to 100 events. Requires a Named Credential named `RFLIB_APP_INSIGHTS` authenticated with Microsoft Entra ID and the `App_Insights_Instrumentation_Key` Global Config value. See `azure/README.md` for the full setup guide. Deployment templates (Bicep) and an Entra app-registration script are provided under `azure/`.
- Added vendor-neutral OpenTelemetry (OTLP/HTTP) log forwarding. Log events at or above the configurable `OTel_Log_Level__c` threshold are forwarded to any OTLP/HTTP JSON logs endpoint (AWS CloudWatch's native OTLP endpoint, an OpenTelemetry collector, or any vendor backend that accepts the JSON encoding) by the new `Send_Log_Event_to_OTel` flow action. Azure Monitor is supported only behind an OpenTelemetry collector, because its native OTLP ingestion requires protobuf (for direct Azure ingestion use the App Insights forwarder above). Each event becomes an OTLP `logRecord` (severity mapped to the OTel `SeverityNumber` scale) under a single `resourceLogs` payload, sent in callouts of up to 100 events. The full logs URL and any backend-specific headers are configured on the `RFLIB_OTEL_LOGS` Named Credential. See `otel/README.md` for setup recipes including the Lambda-free AWS CloudWatch OTLP option.
- Added Google Chat notification support for RFLIB log events. Log events at or above a configurable threshold (minimum WARN) are forwarded to a Google Chat space via the `Send_Log_Event_to_Google_Chat` flow action, with the Google Chat log level controlled by the new `Google_Chat_Log_Level__c` setting and the maximum notifications per invocation configurable as a Global Config value (`Max_GChat_Notifications_Per_Invocation`). Requires a Named Credential named `RFLIB_GOOGLE_CHAT_WEBHOOK` configured with a Google Chat Incoming Webhook (Authentication Protocol: No Authentication — the webhook key and token are part of the URL). The payload is a `cardsV2` card containing the Org Name, log level, created-by, context, request ID, log messages, and platform info; long log messages are truncated from the beginning to preserve the trailing exception detail.
- Added a Log Archive Alert banner to the Management Dashboard home page. The banner summarizes recent WARN, ERROR, and FATAL log events from the Log Archive (Big Object) and links directly to the Log Monitor for investigation. The lookback window (default 24 hours) and the log levels that trigger the alert (default WARN, ERROR, FATAL) are configurable on the component, and the banner stays hidden when there is nothing to report. When the lookback window is too large to summarize fully, the banner shows a partial-count advisory so a real alert is never silently suppressed.
